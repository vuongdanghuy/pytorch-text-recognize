...
class ModelBuilder(nn.Module):
  ...
  def __init__(self, arch, rec_num_classes, sDim, attDim, max_len_labels, eos, STN_ON=False):
    ...
    self.eos = eos
    self.STN_ON = STN_ON
    self.tps_inputsize = config['tps_inputsize']

    self.encoder = create(self.arch,
                      with_lstm=config['with_lstm'],
    ...
    if self.STN_ON:
      self.tps = TPSSpatialTransformer(
        output_image_size=tuple(config['tps_outputsize']),
        num_control_points=config['num_control_points'],
        margins=tuple(config['tps_margins']))
      self.stn_head = STNHead(
        in_planes=3,
        num_ctrlpoints=config['num_control_points'],
        activation=config['stn_activation'])
  ...
  def forward(self, input_dict):
    ...
    if self.training:
      ...
      rec_pred, rec_pred_scores = self.decoder.beam_search(encoder_feats, config['beam_width'], self.eos)
